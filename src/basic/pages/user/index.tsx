...
import { CouponActions, CartActions, NotificationActions, Product, SOLD_OUT, ProductsActions, AppState } from "../../shared/types";
...
export const UserPage = ({
  ...
  const { cart, addToCart, getCartItem, removeFromCart, updateQuantity, calculateItemTotal, applyCoupon, calculateCartTotal, clearCart } = cartActions; 
  ...
  const formatPrice = (price: number, productId?: string): string => {
    ...
    return product && getRemainingStock(product.stock) <= 0 ? SOLD_OUT : formatUserPrice(price);
  }
        ...
        <section>
          ...
          {filteredProducts.length === 0 ? (
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
              ...
              const remainingStock = getRemainingStock(product.stock, getCartItem(product.id)?.quantity);
